...
basicplot=ggplot(rt,aes(d=group,m=circEGFR,col="red"))+geom_roc(n.cuts = 0)
#pdf('riskScore_clinical_plotROC.pdf',width = 10,height = 8)
basicplot+style_roc()
#dev.off()
graph2ppt(file="response_ROC.pptx", width=7, height=6)

#styleplot          #??ROC
roc=roc(rt$group,rt$circEGFR,plot=FALSE,ci=TRUE)
auc=auc(roc)          #????AUC?? 
